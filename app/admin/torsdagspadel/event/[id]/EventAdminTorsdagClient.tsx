...
export default function EventAdminTorsdagClient({ eventId }: { eventId: string }) {
  ...
  const [signups, setSignups] = useState<
    { visningsnavn: string; elo: number; tidligste_tid: string; profileId?: string }[]
  >([]);
    ...
    try {
      ...
      const mapped = rows
        .map((r) => {
          ...
          const tid = (r.tidligste_tid || "").slice(0, 5) || "—";
          return { visningsnavn: vn, elo, tidligste_tid: tid, profileId: profile?.id };
        })
        .filter(Boolean) as { visningsnavn: string; elo: number; tidligste_tid: string; profileId?: string }[];

      // Sortér efter tidligste tid (ukendte sidst)
      mapped.sort((a, b) => {
        const ta = a.tidligste_tid === "—" ? "99:99" : a.tidligste_tid;
        const tb = b.tidligste_tid === "—" ? "99:99" : b.tidligste_tid;
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 sm:gap-4">
        ...
        >
          ...
          <div className="mt-3">
            ...
            <div
              className="mt-2 rounded border bg-white dark:bg-zinc-900"
              style={{ borderColor: GREEN }}
            >
              <div
                ...
                style={{ borderColor: GREEN, color: GREEN }}
              >
                Tilmeldinger {event?.date ? `(${event.date})` : ""}
              </div>
              {loadingSignups ? (
                <div className="p-2 text-sm opacity-70">Indlæser…</div>
              ) : signups.length === 0 ? (
                <div className="p-2 text-sm opacity-70">Ingen (endnu).</div>
              ) : (
                <ul className="max-h-56 overflow-auto divide-y dark:divide-zinc-800">
                  {signups.map((s) => {
                    const exists = allProfiles.find(
                      (p) => (p.visningsnavn || "").trim() === s.visningsnavn
                    );
                    const inEvent = orderedPlayers.some(
                      (p) => (p.visningsnavn || "").trim() === s.visningsnavn
                    );
                    return (
                      <li key={s.visningsnavn} className="px-3 py-2 text-sm flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          <div className="truncate font-medium">
                            {s.visningsnavn} <span className="opacity-70">· ELO {s.elo}</span>
                          </div>
                          <div className="text-[11px] opacity-70">Tidligste: {s.tidligste_tid}</div>
                        </div>
                        <button
                          type="button"
                          disabled={!exists?.id || inEvent || locked}
                          onClick={() => exists?.id && addPlayer(exists.id, s.visningsnavn)}
                          className={`px-2 py-1 rounded border text-xs ${
                            !exists?.id || inEvent || locked
                          ...
                          title={
                            inEvent
                              ...
                              : locked
                              ? "Programmet er låst"
                              : "Tilføj spiller til event"
                          }
                        >
                          {inEvent ? "✓ Tilføjet" : exists?.id ? "Tilføj" : "Ukendt profil"}
                        </button>
                      </li>
                    );
                  })}
...
function CenterMatches({
  ...
  const ScoreBox = ({
    ...
    <input
      ...
      className="w-7 border rounded px-0.5 py-0.5 text-center text-sm tabnums bg-white dark:bg-zinc-900"
      style={{ borderColor: GREEN }}
      title={title}
    />
  );
                ...
                return (
                  <div key={key} className="flex items-center justify-between gap-2 text-sm">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span className="font-semibold tabnums min-w-[36px] text-right" style={{ color: GREEN }}>
                        {plusTxt}
                      </span>
                    </div>
                  </div>
